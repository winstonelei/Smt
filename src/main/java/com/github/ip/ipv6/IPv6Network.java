...
/**
 * Immutable representation of an IPv6 network based on an address and a prefix length. An IPv6 network is also an IPv6 address range (but not all ranges are
 * valid networks).
 * 
...
public final class IPv6Network extends IPv6AddressRange {
     ...
     * 
     * @param address
     *            IPv6 address (the network address or any other address within the network)
     * @param networkMask
     *            IPv6 network mask
     * @return IPv6 network
     */
    ...
    /**
     * Create an IPv6 network from the two addresses within the network. This will construct the smallest possible network ("longest prefix length") which
     * contains both addresses.
     * 
     * @param one
     *            address one
     * @param two
     *            address two, should be bigger than address one
     * @return ipv6 network
     */
     ...
     * 
     * @param size
     *            size (expressed as {@link com.googlecode.ipv6.IPv6NetworkMask}) of the subnets
     * @return iterator of the splitted subnets.
     * @throws IllegalArgumentException
     *             if the requested size is bigger than the original size
     */
    public Iterator<IPv6Network> split(IPv6NetworkMask size) {
        if (size.asPrefixLength() < this.getNetmask().asPrefixLength())
            throw new IllegalArgumentException(String.format("Can not split a network of size %s in subnets of larger size %s", this.getNetmask().asPrefixLength(),
                    size.asPrefixLength()));
    ...
    public boolean equals(Object o) {
        ...
        if (address != null ? !address.equals(that.address) : that.address != null)
            return false;
        if (networkMask != null ? !networkMask.equals(that.networkMask) : that.networkMask != null)
            return false;
